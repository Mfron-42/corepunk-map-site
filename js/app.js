...
/* Statistiques de monstre (stats_decoded/stat_curve, voir openMonsterFiche
   plus bas) et type de coffre/objet activable (world_objects.json
   chest_type/activable_type, voir openChestFiche/popupHtml) — mêmes tokens
   neutres moteur, même principe "le site possède le libellé localisé". */
const statLabel = key => tbl('statLabel', key) || pretty(key);
const statTierLabel = key => tbl('statTier', key) || pretty(key);
...
async function loadCritical() {
  ...
  S.items = items;
  buildRarityGroups();   // regroupe les variantes de rareté « même nom » (voir buildRarityGroups)
...
function popupHtml(cat, r, id) {
  ...
  // Was leaking the raw internal entity key (e.g. "qao_radio_red") straight
  // into the popup -- replaced by the localized activable_type badge, same
  // "site owns the label" principle as chest below (see data/SCHEMA.md
  // "Activable type").
  ...
  return `<div class="pop">
    ...
    <div class="pop-cat" style="color:${c.hex}">${esc(catLabel(cat))}${typeBadge ? ' ' + typeBadge : ''}</div>
...
/* Fiche coffre (S.data.chest, tc_* placements) : type physique (badge, même
   principe que openCampFiche) + butin (même rendu monsterLootRow/
   lootRowsHtml que monstre/camp) + bouton carte. Pas de lien profond dédié
   (setFicheHash(null)) — même traitement que openLocationFiche/
   openAbilityFiche : ~3830 marqueurs individuels par skin de prop, pas un
   id stable qu'un lien partagé devrait rouvrir (voir data/SCHEMA.md "Chest
   loot + type" sur pourquoi ces marqueurs ne sont pas individuellement
   indexés en recherche non plus). */
...
/* Statistiques de monstre — voir data/SCHEMA.md "Monster stats". Seuls
   ...
   n'ont AUCUNE stat par-monstre côté client (stats_source null) — le
   client ne nomme jamais quel tier de difficulté un mob donné utilise en
   jeu. On estime alors avec la courbe d'UN template représentatif par tier
   ...
   honnête, jamais un chiffre par-monstre fabriqué — d'où le badge "≈
   estimé (tier X)" toujours visible à côté, jamais présenté avec la même
...
document.addEventListener('click', e => {
  ...
  if (['fiche-quest', 'fiche-npc', 'fiche-camp', 'fiche-item', 'fiche-monster', 'fiche-chest', 'goto'].includes(b.dataset.act)) pushFocusState();
...
/* Chip QUANTIFIÉE ({key, count}) — même rendu que itemChip mais avec un
   ...
   toujours donnés). Partagée entre les deux au lieu de dupliquer le rendu :
   la forme {key, count} est identique dans les deux cas (voir() pour les ingrédients et pour les récompenses fixes). */
...
/* Récompenses de quête : distingue TOUJOURS DONNÉ (xp/or + items à
   ...
   (ex. allergic_to_duty : 6 items toujours donnés + 4 groupes "choisissez
   1 parmi 3"). qtyItemChip/qtyChipList (section recette) gèrent déjà la
   forme {key,count} — réutilisées telles quelles ici. */
...
function openItemFiche(key) {
  ...
  // variantes « même nom » (voir buildRarityGroups), une pastille par rareté ;
  // celle de la fiche courante est allumée, les autres rouvrent leur variante
  // (data-act=fiche-item -> openItemFiche, qui met à jour prix/butin/hash).
...
/* ── Variantes de rareté « même nom, une clé par rareté » ──────────────
   Certains objets existent comme N enregistrements DISTINCTS partageant le
   MÊME nom d'affichage, ne différant que par leur rareté encodée en DERNIER
   token de la clé : synthesis_item_upgrade_t1_uncommon / _rare / _epic sont
   tous « Synthesis Item Upgrade T1 ». Sans regroupement, la recherche crache
   3+ quasi-doublons et chaque fiche est un cul-de-sac (aucun moyen de passer
   à l'épique). On calcule côté client (aucune donnée regénérée) deux tables
   sur S.items : rarityGroups (repKey -> {baseName, variants:{Rareté: clé}})
   et rarityGroupOf (chaque membre, représentant compris, -> repKey).
   À NE PAS confondre avec le craft pondéré (art_t3_* : UNE seule clé, tirage
   RNG, raretés dans it.rarities) — ces items-là ont une clé unique, ne
   finissent pas par un token de rareté, donc ne tombent jamais dans un groupe
   (voir data/SCHEMA.md items.json / recipes.json). */
...
/* Groupe de rareté d'une clé item (undefined si l'item n'en fait pas partie).
...
function buildSearch() {
  ...
  Object.entries(S.items).forEach(([key, it]) => {
    ...
    // (b) membre non-représentant d'un groupe de rareté : une seule entrée par
    // groupe (le représentant), la rareté se choisit sur la fiche.
    const grp = rarityGroupFor(key);
...
function buildChestSearchIndex() {
  ...
  seen.forEach(r => {
    ...
    // même si le libellé affiché (chestSearchLabel) ne le cite pas.
    ...
    pushSearchEntry(chestSearchLabel(r.name), 'chest', CATS.chest.hex, r.x, r.z,
      null, null, typeLabel, null, 0, body);
  });
}
...
function renderSearch(raw) {
  ...
  res.forEach(it => {
    ...
    // Coordonnées ET sous-libellé peuvent coexister (ex. coffre : type +
    // position) — jusqu'ici le sous-libellé d'une entrée POSITIONNÉE
    // (events déjà, chest désormais) n'était jamais montré, les coordonnées
    // l'emportant toujours ; les deux s'affichent maintenant côte à côte.
    const posOrSub = [it.x != null ? fmtCoord(it.x, it.z) : null, it.sub || null].filter(Boolean).join(' · ');
    li.innerHTML = `<div class="sr-row">
      ...
      ${mapBadge}
      <span class="muted">${esc(posOrSub)}</span>
    </div>${hint}`;
    li.onclick = () => {
      pushFocusState();   // avant mutation — voir pushFocusState()'s doc
