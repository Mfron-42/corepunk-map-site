...
export default {
    ui: {
      ...
      abilityParamsNamedTitle: 'Параметры',
      abilityParamsHint: 'Движковые коды клиента, как есть — без перевода в игровые характеристики.',
      engineCodeHint: 'Движковый код — в данных клиента нет игрового названия.',
      ...
      // ПЛОСКИЕ корневые разделы Монстры / Крипы / Мирная фауна (реорг владельца
      // 2026-07-15b: раздел-зонтик «Существа» с вложенными подгруппами отменён —
      // каждый снова становится своим корневым разделом). groupWildlife — это
      // ЗАГОЛОВОК РАЗДЕЛА пула camp:wildlife — УНИФИЦИРОВАН 2026-07-16 на ТО ЖЕ
      // слово, что его строка/слой/легенда/карточка (wildlifeRestRow «Мирные
      // животные»): одно слово везде, больше нет «Мирная фауна» vs «Мирные животные».
      groupCreeps: 'Крипы',
      groupWildlife: 'Мирные животные',
      ...
      // Сворачиваемый родитель, собирающий все динамические строки / строки
      // серверного спавна дерева интерактивных объектов (консолидация, 2026-07-15).
      spawnCampsGroup: 'Зоны спавна (лагеря)',
      ...
      // Строки ВНУТРИ группы «Зоны спавна (лагеря)»: родитель несёт контекст,
      // строка называет лишь свой тип. searchSpotsRow (старая плоская подпись)
      ...
      // разведено 2026-07-16b: «Разные лагеря» (отличается от «Обыск без типа»,
      // бывшего «Прочее» остаточных зон обыска ниже).
      otherCampsRow: 'Разные лагеря',
      // Строки спавна, ТИПИЗИРОВАННЫЕ по доказанному содержимому (2026-07-15):
      // «Трупы» спавна здесь и «Размещённые трупы» в decorFamily — одна и та
      // же концепция в двух честных формах.
      searchSpotsCorpsesRow: 'Трупы',
      // Остаточный бакет `searchable` — разведено 2026-07-16b: «Обыск без типа»
      // (больше не «Прочее», что дублировало «Разные лагеря» бакета `other`).
      searchSpotsOtherRow: 'Обыск без типа',
      ...
      // Переименовано 2026-07-16b («Зоны спавна — прочие (лагеря)» → «Динамические
      // лагеря (спавн)»): родитель больше не «… прочие».
      dynamicCampsGroup: 'Динамические лагеря (спавн)',
      ...
      // Блоки цели «труп/контейнер задания» (переработка владельца 2026-07-16b,
      // stepguide.js goalCorpseExtras) — БЕЗ выпадающих меню, максимум ДВЕ строки,
      // по сигналам самих данных (никогда не по жёстко заданному заданию):
      //   1. Размещённые трупы (goalPlacedCorpsesTag / goalPlacementsTag) — badge(official):
      //      ТОЛЬКО точные размещения, честное имя (не завышенная цель);
      //   2. Подсказка 💡 — стандартная ссылка [Трупы(●)] (refKind.corpse),
      //      НАСТОЯЩИЙ переключатель слоёв «Трупы»; подпись goalHintZonesTag,
      //      нюанс (goalHintZonesNote) во всплывающей. 💡 — её метка, не фраза.
      goalSpawnPoolLabel: (name, n) => `Зона спавна — ${name} (${n} т.)`,
      goalCorpsePlacedN: n => `${n} размещённых трупов`,
      // ЧЕСТНЫЕ подписи официального тира (ТОЛЬКО точные размещения): «Размещённые
      // трупы», когда цель принимает типы трупов, иначе «Места».
      goalPlacedCorpsesTag: 'Размещённые трупы',
      goalPlacementsTag: 'Места',
      // Подпись ссылки-подсказки [Трупы(●)] — без «трупов» (сам значок уже несёт
      // kind «Трупы»): ссылка читается как `[Трупы(●)] Зоны спавна`.
      goalHintZonesTag: 'Зоны спавна',
      goalHintZonesNote: 'Любой труп этих типов засчитывается, где угодно на карте.',
    ...
    refKind: {
      ...
      // Группа «Трупы» в дереве (подсказка поиска трупов у цели) — то же слово,
      // что и groupCorps; оттенок задаёт вызывающий код (DECOR_HEX.corpse).
      corpse: 'Трупы',
    },
    ...
    badge: {
      ...
      // Тир «совет игрока» — известно В ИГРЕ из опыта, никогда не доказуемо по
      // извлечённым данным; янтарно-пунктирный тон, отличный от «производного»
      // (переиспользует надпись бывшего playerHintLabel).
      provPlayerKnowledge: 'Совет игрока',
      provPlayerKnowledgeTip: 'Известно в игре из опыта — никогда не доказуемо по извлечённым данным.',
    ...
    decorFamily: {
      ...
      // `corpse` = строка РАЗМЕЩЁННЫХ тел (фиксированная концепция) —
      // «Размещённые трупы», отличная от «Трупы» СПАВНА группы «Зоны спавна
      // (лагеря)»: одна концепция в двух честных формах. Подписи РОЛИ
      // (corpse_quest/loot/decor) остаются: config.js chestKindLabel
      // показывает их ПО ЗАПИСИ в карточке/попапе, а не как строки дерева
      // (config.js corpseRoleKey).
      corpse: 'Размещённые трупы',
