...
export default {
    ui: {
      ...
      groupWorld: 'Мир',
      // Раздел «Существа» (реорг владельца 2026-07-15): объединяет прежние
      // корневые разделы Монстры + Крипы + слой Мирной фауны (пул
      // camp:wildlife) в ОДИН раздел с тремя сворачиваемыми подгруппами.
      // groupMonsters/groupCreeps СОХРАНЕНЫ выше (паритет i18n); подписи
      // подгрупп ниже теперь несут их в дереве.
      groupCreatures: 'Существа',
      subMonstres: 'Монстры',
      subCreeps: 'Крипы',
      subFaunePaisible: 'Мирная фауна',
      ...
      // Блоки цели «труп/контейнер задания» (переработка 2026-07-15, stepguide.js
      // goalCorpseExtras) — переиспользуемая модель из ТРЁХ уровней честности,
      // по сигналам самих данных (никогда не по жёстко заданному заданию):
      //   1. Принятые типы — полный список bound_units, свёрнут в сводку
      //      (goalAcceptedSummary);
      //   2. 🟢 Официальные позиции (goalOfficialPositions) — точные размещения
      //      + пул role="quest"; синяя ссылка данных, рисуемая;
      //   3. 💡 Вероятные места (goalHintLocations) — пулы role="generic",
      //      ВЫВЕДЕННЫЕ по луту: семейство 💡, примечание «подсказка, не
      //      официальные данные» (goalHintLocationsNote);
      //   4. 💡 Совет игрока (playerHintLabel) — известно в игре, не извлечено.
      ...
      // Названия двух уровней позиций + примечание выведенного уровня. Мета
      // уровней переиспользует goalAcceptedMeta (офиц.: «N размещ. + M в зонах»)
      // и goalSpawnAggMeta (подсказка: «M т. · N зон»). goalTierDetailN — сводка
      // ящика «подробно» (каждый источник рисуется по отдельности).
      goalOfficialPositions: 'Официальные позиции',
      goalHintLocations: 'Вероятные места',
      goalHintLocationsNote: 'места, выведенные по луту — подсказка, не официальные данные',
      // Слой 2 рисуется, ТОЛЬКО если сужает поиск (stepguide.js hintSpread):
      // покрывающий почти всю карту пул (напр. lost_crew, 9 зон / 8 273 т.)
      // сворачивается в эту краткую заметку (без облака точек) и отсылает к
      // подсказке игрока; ящик «показать N зон» остаётся свёрнутым.
      goalHintWidespread: 'эти позиции покрывают почти всю карту',
      goalHintSeeTip: 'см. подсказку игрока ниже',
      goalHintZonesDetail: n => `показать ${n} зон`,
      goalTierDetailN: n => `подробно (${n})`,
      goalSpawnAggMeta: (pts, zones) => `${pts} т. · ${zones} зон`,
