...
export default {
    ui: {
      ...
      // Блоки цели «труп/контейнер задания» (переработка 2026-07-16, stepguide.js
      // goalCorpseExtras) — БЕЗ выпадающих меню, максимум ДВЕ рисуемые метки, по
      // сигналам самих данных (никогда не по жёстко заданному заданию):
      //   Тир каждой метки помечен фишкой badge() (ЕДИНЫЙ словарь честности
      //   приложения), никогда не эмодзи из гайда:
      //   1. Принятые типы — строка «N тип.» (goalAcceptedSummary);
      //   2. Позиции — badge(official) — ОДНА ссылка, НАЗВАННАЯ реальной целью
      //      (target.label): объединение точных размещений + пулов role="quest";
      //   3. Зоны обыскиваемых трупов (goalHintZonesTag) — badge(derived) — ОДНА
      //      ссылка: объединение пулов role="generic", как «Мирные животные»;
      //   4. Совет игрока — badge(player_knowledge, provPlayerKnowledge): известно
      //      в игре, не в данных (метка несёт название тира).
      goalAcceptedTypesLabel: 'Принятые типы:',
      goalAcceptedSummary: n => `${n} тип.`,
      goalSpawnPoolLabel: (name, n) => `Зона спавна — ${name} (${n} т.)`,
      goalCorpsePlacedN: n => `${n} размещённых трупов`,
      // goalHintZonesTag: ВЫВЕДЕННАЯ метка (покрывает карту, одна метка).
      // goalPositions: только ЗАЩИТНЫЙ запасной вариант — официальная метка
      // названа реальной целью (officialTierLabel/target.label), никогда этим
      // общим словом; сохранён лишь для (не наблюдавшегося) случая без label.
      goalPositions: 'Позиции',
      goalHintZonesTag: 'Зоны обыскиваемых трупов',
