...
export default {
    ui: {
      ...
      // Размещённые (фиксированные) трупы + агрегат зон поиска
      // (goalSpawnPoolAggregate) — одна линия на все пулы спавна, детали в
      // свёрнутом ящике. Итог — сумма данных по пулам.
      goalCorpsePlacedN: n => `${n} размещённых трупов`,
      goalSpawnAggLabel: 'Зоны поиска — трупы (спавн)',
      goalSpawnAggMeta: (pts, zones) => `${pts} т. · ${zones} зон`,
      goalSpawnAggRoles: (q, g) =>
        q && g ? `в т.ч. ${q} зон задания + ${g} доказанных общих зон`
          : q ? `${q} зон задания`
