...
export default {
    ui: {
      ...
      // Блоки цілі «труп/контейнер завдання» (переробка 2026-07-16, stepguide.js
      // goalCorpseExtras) — БЕЗ випадних меню, щонайбільше ДВІ придатні до
      // малювання мітки, за сигналами самих даних (ніколи не за жорстко заданим
      // завданням):
      //   Тир кожної мітки позначено фішкою badge() (ЄДИНИЙ словник чесності
      //   застосунку), ніколи не емодзі з гайда:
      //   1. Прийняті типи — рядок «N тип.» (goalAcceptedSummary);
      //   2. Позиції — badge(official) — ОДНЕ посилання, НАЗВАНЕ реальною ціллю
      //      (target.label): об'єднання точних розміщень + пулів role="quest";
      //   3. Зони обшукуваних трупів (goalHintZonesTag) — badge(derived) — ОДНЕ
      //      посилання: об'єднання пулів role="generic", як «Мирні тварини»;
      //   4. Порада гравця — badge(player_knowledge, provPlayerKnowledge): відоме
      //      у грі, не в даних (мітка несе назву тиру).
      ...
      // goalHintZonesTag: ВИВЕДЕНА мітка (покриває карту, одна мітка).
      // goalPositions: лише ЗАХИСНИЙ запасний варіант — офіційна мітка названа
      // реальною ціллю (officialTierLabel/target.label), ніколи цим загальним
      // словом; збережено лише для (не спостережуваного) випадку без label.
      goalPositions: 'Позиції',
      goalHintZonesTag: 'Зони обшукуваних трупів',
