...
export default {
    ui: {
      ...
      groupWorld: 'Світ',
      // Розділ «Істоти» (реорг власника 2026-07-15): об'єднує колишні кореневі
      // розділи Монстри + Кріпи + шар Мирної фауни (пул camp:wildlife) в ОДИН
      // розділ із трьома згортуваними підгрупами. groupMonsters/groupCreeps
      // ЗБЕРЕЖЕНО вище (паритет i18n); підписи підгруп нижче тепер несуть їх
      // у дереві.
      groupCreatures: 'Істоти',
      subMonstres: 'Монстри',
      subCreeps: 'Кріпи',
      subFaunePaisible: 'Мирна фауна',
      ...
      // Блоки цілі «труп/контейнер завдання» (переробка 2026-07-15, stepguide.js
      // goalCorpseExtras) — багаторазова модель із ТРЬОХ рівнів чесності, за
      // сигналами самих даних (ніколи не за жорстко заданим завданням):
      //   1. Прийняті типи — повний список bound_units, згорнутий у зведення
      //      (goalAcceptedSummary);
      //   2. 🟢 Офіційні позиції (goalOfficialPositions) — точні розміщення +
      //      пул role="quest"; синє посилання даних, придатне до малювання;
      //   3. 💡 Ймовірні місця (goalHintLocations) — пули role="generic",
      //      ВИВЕДЕНІ за лутом: родина 💡, примітка «підказка, не офіційні дані»
      //      (goalHintLocationsNote);
      //   4. 💡 Порада гравця (playerHintLabel) — відоме у грі, не вилучене.
      ...
      // Назви двох рівнів позицій + примітка виведеного рівня. Мета рівнів
      // повторно використовує goalAcceptedMeta (офіц.: «N розміщ. + M у зонах»)
      // і goalSpawnAggMeta (підказка: «M т. · N зон»). goalTierDetailN — зведення
      // шухляди «детально» (кожне джерело малюється окремо).
      goalOfficialPositions: 'Офіційні позиції',
      goalHintLocations: 'Ймовірні місця',
      goalHintLocationsNote: 'місця, виведені за лутом — підказка, не офіційні дані',
      // Шар 2 малюється, ЛИШЕ якщо звужує пошук (stepguide.js hintSpread): пул,
      // що покриває майже всю карту (напр. lost_crew, 9 зон / 8 273 т.),
      // згортається в цю коротку примітку (без хмари точок) і відсилає до
      // підказки гравця; шухляда «показати N зон» лишається згорнутою.
      goalHintWidespread: 'ці позиції охоплюють майже всю карту',
      goalHintSeeTip: 'див. підказку гравця нижче',
      goalHintZonesDetail: n => `показати ${n} зон`,
      goalTierDetailN: n => `детально (${n})`,
