...
export default {
    ui: {
      ...
      // Картка здібності — рядок перезарядки (a.cooldown) + ефект/контроль, сирі параметри, походження-монстр.
      ...
      abilityEffectTitle: 'Ефект',
      abilityEffectDurationLabel: 'Тривалість',
      abilityParamsTitle: 'Сирі параметри',
      abilityParamsNamedTitle: 'Параметри',
      abilityParamsHint: 'Рушійні коди клієнта, як є — без перекладу в ігрові характеристики.',
      engineCodeHint: 'Рушійний код — у даних клієнта немає ігрової назви.',
      abilityOriginMonster: 'Здатність монстра',
      ...
      // ПЛОСКІ кореневі розділи Монстри / Кріпи / Мирна фауна (реорг власника
      // 2026-07-15b: розділ-парасольку «Істоти» з вкладеними підгрупами скасовано —
      // кожен знову стає своїм кореневим розділом). groupWildlife — це ЗАГОЛОВОК
      // РОЗДІЛУ пула camp:wildlife — УНІФІКОВАНО 2026-07-16 на ТЕ САМЕ слово, що
      // його рядок/шар/легенда/картка (wildlifeRestRow «Мирні тварини»): одне
      // слово скрізь, більше немає «Мирна фауна» vs «Мирні тварини».
      groupCreeps: 'Кріпи',
      groupWildlife: 'Мирні тварини',
      ...
      // Згортуваний батько, що збирає всі динамічні рядки / рядки серверного
      // спавну дерева інтерактивних об'єктів (консолідація, 2026-07-15).
      spawnCampsGroup: 'Зони спавну (табори)',
      ...
      // Рядки ВСЕРЕДИНІ групи «Зони спавну (табори)»: батько несе контекст,
      // рядок називає лише свій тип. searchSpotsRow (стара пласка підпис)
      ...
      // розведено 2026-07-16b: «Різні табори» (відмінний від «Обшук без типу»,
      // колишнього «Інше» залишкових зон обшуку нижче).
      otherCampsRow: 'Різні табори',
      // Рядки спавну, ТИПІЗОВАНІ за доведеним вмістом (2026-07-15): «Трупи»
      // спавну тут і «Розміщені трупи» в decorFamily — одна концепція у двох
      // чесних формах.
      searchSpotsCorpsesRow: 'Трупи',
      // Залишковий бакет `searchable` — розведено 2026-07-16b: «Обшук без типу»
      // (більше не «Інше», що дублювало «Різні табори» бакета `other`).
      searchSpotsOtherRow: 'Обшук без типу',
      ...
      // Перейменовано 2026-07-16b («Зони спавну — інші (табори)» → «Динамічні
      // табори (спавн)»): батько більше не «… інші».
      dynamicCampsGroup: 'Динамічні табори (спавн)',
      ...
      // Блоки цілі «труп/контейнер завдання» (переробка власника 2026-07-16b,
      // stepguide.js goalCorpseExtras) — БЕЗ випадних меню, щонайбільше ДВА рядки,
      // за сигналами самих даних (ніколи не за жорстко заданим завданням):
      //   1. Розміщені трупи (goalPlacedCorpsesTag / goalPlacementsTag) — badge(official):
      //      ЛИШЕ точні розміщення, чесне ім'я (не завищена ціль);
      //   2. Підказка 💡 — стандартне посилання [Трупи(●)] (refKind.corpse),
      //      СПРАВЖНІЙ перемикач шарів «Трупи»; підпис goalHintZonesTag, нюанс
      //      (goalHintZonesNote) у спливній. 💡 — її позначка, а не фраза.
      goalSpawnPoolLabel: (name, n) => `Зона спавну — ${name} (${n} т.)`,
      goalCorpsePlacedN: n => `${n} розміщених трупів`,
      // ЧЕСНІ підписи офіційного тиру (ЛИШЕ точні розміщення): «Розміщені трупи»,
      // коли ціль приймає типи трупів, інакше «Місця».
      goalPlacedCorpsesTag: 'Розміщені трупи',
      goalPlacementsTag: 'Місця',
      // Підпис посилання-підказки [Трупи(●)] — без «трупів» (сам значок уже несе
      // kind «Трупи»): посилання читається як `[Трупи(●)] Зони спавну`.
      goalHintZonesTag: 'Зони спавну',
      goalHintZonesNote: 'Будь-який труп цих типів зараховується, будь-де на карті.',
    ...
    refKind: {
      ...
      // Група «Трупи» в дереві (підказка пошуку трупів у цілі) — те саме слово,
      // що й groupCorps; відтінок задає викликач (DECOR_HEX.corpse).
      corpse: 'Трупи',
    },
    ...
    badge: {
      ...
      // Тир «порада гравця» — відоме У ГРІ з досвіду, ніколи не доказовне за
      // витягнутими даними; бурштиново-пунктирний тон, відмінний від «похідного»
      // (перевикористовує напис колишнього playerHintLabel).
      provPlayerKnowledge: 'Порада гравця',
      provPlayerKnowledgeTip: 'Відоме у грі з досвіду — ніколи не доказовне за витягнутими даними.',
    ...
    decorFamily: {
      ...
      // `corpse` = рядок РОЗМІЩЕНИХ тіл (фіксована концепція) — «Розміщені
      // трупи», відмінний від «Трупи» СПАВНУ групи «Зони спавну (табори)»: одна
      // концепція у двох чесних формах. Підписи РОЛІ (corpse_quest/loot/decor)
      // лишаються: config.js chestKindLabel показує їх ЗА ЗАПИСОМ у картці/
      // попапі, а не як рядки дерева (config.js corpseRoleKey).
      corpse: 'Розміщені трупи',
